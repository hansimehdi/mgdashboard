...
export class UserSettingsComponent implements OnInit {
  ...
  updateDetails() {
    if (this.detailForm.valid) {
      const files = this.ref.nativeElement.querySelector('#cv').files;
      if (files && files[0]) {
        this.showCVMissing = true;
        this.showUploadProgress = true;
        const formData: FormData = new FormData();
        formData.append('cv', files[0]);
        formData.append('id', this.details.id);
        formData.append('cvUrl', this.details.cvUrl);
        formData.append('domain', this.details.domain);
        formData.append('accId', this.details.accId);
        this.accountManager.updateDetails(formData).subscribe(
          data => {
            this.showUploadProgress = false;
            switch (data.response) {
              case 'OK':
  ...
  buidDetails() {
    this.showProgress = true;
    this.accountManager.getUserDetail().subscribe(
      data => {
        this.showProgress = false;
