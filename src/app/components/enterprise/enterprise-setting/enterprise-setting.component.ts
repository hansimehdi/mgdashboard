...
import { SlideInOutAnimation } from 'src/app/classes/animation/fadeInOut/fade-in-out';
...
@Component({
  ...
  styleUrls: ['./enterprise-setting.component.css'],
...
export class EnterpriseSettingComponent implements OnInit {
  ...
  public details: Enterprise;
  public animationState: string;
  public animationStateReverse: string;
  public showUpdateButton: boolean;
  public detailForm: FormGroup;
  public showCVMissing: boolean;
  constructor(
  ...
  updateDetails() {
    ...
    if (this.detailForm.valid) {
      this.enterpriseManager.update(this.details).subscribe(
        data => {
          this.showProgress = false;
          this.buidDetails();
          this.toggleShowDiv();
  ...
  buidDetails() {
    this.showProgress = true;
    this.details = new Enterprise();
    this.enterpriseManager.current().subscribe(
      data => {
        this.showProgress = false;
        this.details = data.enterprise;
        this.detailForm = new FormGroup({
          'domain': new FormControl(this.details.domain, Validators.required),
          'trn': new FormControl(this.details.trn, Validators.required),
          'empNum': new FormControl(this.details.empNum, Validators.required),
          'address': new FormControl(this.details.address),
          'capital': new FormControl(this.details.capital)
        });
        this.detailForm.valueChanges.subscribe(
          value => {
            if (value.domain && value.domain !== '') {
              this.details.domain = value.domain;
            }
            if (value.trn && value.trn !== '') {
              this.details.trn = value.trn;
            }
            if (value.empNum && value.empNum !== '') {
              this.details.empNum = value.empNum;
            }
            if (value.address && value.address !== '') {
              this.details.address = value.address;
            }
            if (value.capital && value.capital !== '') {
              this.details.capital = value.capital;
            }
          }
        );
      },
